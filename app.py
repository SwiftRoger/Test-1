...
def check_guess():
    ...
    if user_guess > number_to_guess:
        ...
        result_label.config(
            text=f"Congratulations! You've guessed the number {number_to_guess} in {attempts} attempts."
        )
        guess_entry.config(state="disabled")
        guess_button.config(state="disabled")
        try_again_button.pack(pady=10)  # Show the Try Again button

# Function to handle the Try Again button
def try_again():
    guessing_game()  # Restart the game
